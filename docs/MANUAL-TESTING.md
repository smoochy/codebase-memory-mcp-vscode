...
<!-- Generated from docs/manual-testing-rows.json and docs/MANUAL-TESTING.template.md by scripts/manual-testing-doc.mjs. Edit those, then run `npm run docs:rows`; `npm run check:rows` fails when this file and the registry disagree. -->
...
**1.1.2 carries no re-run of the rows below, and the check it does need is already automated.** It changes where the head commit of an indexed project comes from - `git rev-parse` instead of the git extension's `openRepository` - so what has to hold is that indexed projects stay out of the Source Control view while stale detection still works. Both are asserted by `test/integration/git`, which runs a second VS Code launch with extensions enabled because the rest of the tier runs with them disabled and cannot see this at all. It was also checked against packaged `.vsix` builds of 1.1.1 and this version in a clean profile with 16 indexed projects: 16 repositories listed before, 1 after. Nothing in the panel changed, so the sign-offs below continue to speak for it.
...
The rows a release still needs a person for, generated from the registry rather than remembered. `Blocking` rows have to be run before the release goes out; `Important` and `Nice to have` rows may be skipped, but only with a sign-off recorded here saying they were. The maintainer authorises the release and owns that sign-off.
