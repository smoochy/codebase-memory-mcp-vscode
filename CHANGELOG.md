...
- The last two automatable rows of the manual checklist run unattended now, and nothing about the installed extension changes with them. Row B4 points `binarySource` at the pinned fixture binary and asserts the panel reports the source as external, offers neither Setup nor Update, and still renders its projects section - the source assertion is what keeps a missing binary, which resolves to the setup screen, from satisfying the row for the wrong reason. Row B18 indexes a throwaway git checkout, asserts the card names the branch it was indexed from, then commits in it and polls the panel markup until the project is reported outdated; the markup is the observable rather than the log line, which says "outdated" only once per change. The checkout's real path is resolved before it is added, because on Windows the temporary directory is handed out in its 8.3 short form while the CLI reports the long form back, and the extension matches the two by string - so the project would get no index note and its staleness could never be decided. Both tests are pending, never passing, when the pinned fixture is absent.

- The first-start row of the manual checklist runs unattended now, against the packaged extension rather than the checkout, and nothing about the installed extension changes with it. A fifth Electron pass installs the `.vsix` this manifest's version names into its own scratch profile and launches without `--disable-extensions`, so what activates is the artifact a user would install. It asserts what a first start actually promises - activation without error, every declared command registered, the declared setting defaults read back, the MCP server definition provider contributed - and asserts first that the development-mode test seams are absent, because their presence would mean the pass was reading the checkout again and every assertion after it would be worthless. The pass is left out entirely when no matching package exists, and a suite that does not run has its previous verdict deleted rather than left behind for the row checks to read. The development path it launches with points at an empty directory: dropping the flag altogether leaves an ordinary window that loads the extension, runs no test and never exits.

- The three update-path rows of the manual checklist run unattended now, and nothing about the installed extension changes with them. A fourth Electron pass, launched last and only when the pinned fixture exists, downgrades the managed installation to the pinned older build and updates back out of it: once with the old daemon running, once asserting that daemon is gone afterwards and a fresh call against the new build works, and once - on Windows - holding the target with a `FileShare.Read` handle so the rollback fires and the older installation stays byte-identical with no staged file left behind. The suite gets its own launch because those rows replace the one binary every other suite shares and a daemon outlives the window that started it. The environment handed to the test host now drops `~/.local/bin` from `PATH` when a scratch home is in play, since a developer's own CLI there resolves as an external install and the extension then refuses to update at all. A row may be scoped to one operating system: its test is registered only there, the drift check no longer demands the identifier elsewhere, and every run says which rows its platform left unchecked. The suite runs the extension's own Setup once before the rows, because the extension updates only an installation it recorded installing and a fixture provisioned behind its back is refused; and it clears the CLI's own state directories rather than the scratch home's whole `AppData/Local`, which Windows writes to continuously.
